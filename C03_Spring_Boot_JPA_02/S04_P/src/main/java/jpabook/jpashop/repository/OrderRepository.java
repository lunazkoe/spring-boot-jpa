...
//            .setMaxResults(100)
    ...
    // - Hibernate 6 부터는 자동으로 중복된 데이터(객체)를 제거 해줌 (자동 distinct 적용)
    // - DB의 distinct는 완전히 동일한 행에 대해서만 적용됨

    // 이건 페이징 처리해도 문제 없음 ToOne 관계
    ...
    public List<Order> findAllWithOrderMember(int offset, int limit) {
        return em.createQuery(
                "select distinct o from Order o" +
                    " join fetch o.member m" +
                    " join fetch o.delivery d", Order.class)
